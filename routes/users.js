...
router.get('/', function (req, res) {
    var queryParams = getParsedParamObject(req);
...
router.get('/:id', function (req, res) {
    var queryParams = getParsedParamObject(req);
    var query = userSchema.findById(req.params.id);
    applyQueryParamsToQuery(query, queryParams);
...
router.get('/profilePicUrl/:id', function (req, res) {
    ...
    query.exec(function (err, user) {
        if (err || user === null) {
            var errorMessage = "Profile pic not found";
            sendError(res, errorMessage, 404);
        } else {
            res.status(200).send({
                message: 'OK',
...
router.get('/favorites/:id', function (req, res) {
    ...
    query.exec(function (err, user) {
        if (err || user === null) {
            var errorMessage = "Favorites not found";
            sendError(res, errorMessage, 404);
        } else {
            res.status(200).send({
                message: 'OK',
...
router.get('/following/:id', function (req, res) {
    ...
    query.exec(function (err, user) {
        if (err || user === null) {
            var errorMessage = "Following users not found";
            sendError(res, errorMessage, 404);
        } else {
            res.status(200).send({
                message: 'OK',
...
router.get('/followers/:id', function (req, res) {
    ...
    query.exec(function (err, user) {
        if (err || user === null) {
            var errorMessage = "Followers not found";
            sendError(res, errorMessage, 404);
        } else {
            res.status(200).send({
                message: 'OK',
    ...
    query.exec(function (err, user) {
        if (err) {
            ...
            user.save(function (err, user) {
                if (err) {
                    var errorMessage = "Couldn't add profile pic";
                    sendError(res, errorMessage, 404);
                } else {
                    res.status(200).send({
...
router.put('/favoriteRecipe/:id', function (req, res) {
    ...
    query.exec(function (err, user) {
        if (err) {
            ...
            user.save(function (err, user) {
                if (err) {
                    var errorMessage = "Favorite not found";
                    sendError(res, errorMessage, 404);
                } else {
                    res.status(200).send({
    ...
    userSchema.findById(req.params.id, function (err, user) {
        if (err || user === null) {
            ...
            userSchema.remove({ "_id": mongoose.Types.ObjectId(req.params.id) }, function (err, user) {
                if (err) {
                    var _errorMessage = "User not found";
                    sendError(res, _errorMessage, 404);
                } else {
                    res.status(200).send({
